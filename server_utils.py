...
import logging
from constants.server import HOST_PATH, PORTA_SERVER, PREDICT_FUNCTION, GRAYSCALE_FUNCTION
...
def grayscale_server(file_path: str) -> (str, float):
    ...
    response = requests.post(HOST_PATH + PORTA_SERVER + GRAYSCALE_FUNCTION, files=files)
    logger.debug("immagine inviata al server")
